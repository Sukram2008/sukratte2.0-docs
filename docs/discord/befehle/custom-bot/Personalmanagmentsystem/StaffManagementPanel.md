---
title: Staff-Management Panel
description: Öffnet das grafische Verwaltungs-Panel für ein Teammitglied
keywords: [Staff-Management, Panel, Admin, Management, Owner, Übersicht]
---

# 🖥️🎭 | Staff-Management Panel

Befehl: **`/staff-management panel`**

:::danger Management-Sperre
Dieses Panel gewährt Zugriff auf alle administrativen Funktionen. Die Ausführung ist systemseitig **ausschließlich für den Owner** freigeschaltet.
:::

### Andere Optionen
* **`user:`** Das Teammitglied, dessen Panel geöffnet werden soll.

### Berechtigungen
<div style={{display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '10px'}}>
  <strong style={{minWidth: '80px'}}>👥 Rollen:</strong>
  <div style={{display: 'flex', flexWrap: 'wrap', gap: '8px'}}>
    <span style={{backgroundColor: '#aa3a3a', color: '#fff', padding: '2px 8px', borderRadius: '4px', fontSize: '0.85em'}}>👑 | Owner</span>
  </div>
</div>
<div style={{display: 'flex', alignItems: 'center', gap: '10px'}}>
  <strong style={{minWidth: '80px'}}>📺 Kanäle:</strong>
  <div style={{display: 'flex', flexWrap: 'wrap', gap: '8px'}}>
    <span style={{backgroundColor: '#6e31e0', color: '#fff', padding: '2px 8px', borderRadius: '4px', fontSize: '0.85em'}}>#️⃣ | Alle Kanäle</span>
  </div>
</div>